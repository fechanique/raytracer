    ...
    for(let[index, sector] of sectors.entries()){
        ...
        for(let light of lights){
            if(light.z0 >= sector.z1 && light.z1 <= sector.z1 && light.z <= sector.z1){
                ...
                let max = light.z0 - light.z
                let calc = light.intensity*Math.pow((1-(dist/max)), dist/max)
                let intensity = Math.min(calc, 1)
                sector.ceil_lights.push({intensity, dist:light.dist, x:light.x, y:light.y, z:light.z})
            }
            if(light.z1 <= sector.z0 && light.z0 >= sector.z0 && light.z >= sector.z0){
                let dist = light.z-sector.z0
                let max =  light.z - light.z1
                let calc = light.intensity*Math.pow((1-(dist)/max), dist/max)
                let intensity = Math.min(calc, 1)
                sector.floor_lights.push({intensity, dist:light.dist, x:light.x, y:light.y, z:light.z})
            }
            sector.lights.push([])
        }
    }
