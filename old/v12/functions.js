...
function animateDoor(sector, elapsedTime){
    ...
    for(let [i, p] of sector.original.points.entries()){
        sector.points[i][0] = Math.round((p[0] - sector.params.x)*Math.cos(sector.r0*Math.PI/180) - (p[1] - sector.params.y)*Math.sin(sector.r0*Math.PI/180) + sector.params.x)
        sector.points[i][1] = Math.round((p[0] - sector.params.x)*Math.sin(sector.r0*Math.PI/180) + (p[1] - sector.params.y)*Math.cos(sector.r0*Math.PI/180) + sector.params.y)
    }
}
...
function break_glass(sector){
    console.log('hit', sector.name, sector.hits)
    ...
    if(sector.hits == 3){
        ...
        audio.load('sounds/glass-smash.mp3', sector.x, sector.y, sector.z)
    }
    for(let point of sector.points){
        point[2] = 'broken_glass.jpg'
    }
}