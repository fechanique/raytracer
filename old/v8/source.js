...
function renderFrame(){
    ...
    for(let ray of rays){ //hacer operaciones aquí no quita muchos frames
        ...
        //ray.coll.sort((a, b) => {
        //    if(player_height+jump < a.sector.z0 && player_height+jump < b.sector.z0) return b.sector.z1 - a.sector.z1
        //    if(player_height+jump > a.sector.z1 && player_height+jump > b.sector.z1) return a.sector.z0 - b.sector.z0
        //})
        ray.coll.sort((a, b) => {
            //if(player_height+jump > a.sector.z1 && a.sector.z1 >= b.sector.z0) return -1
            //if(player_height+jump > b.sector.z1 && b.sector.z1 >= a.sector.z0) return 1
            ...
            //if(player_height+jump < b.sector.z0 && b.sector.z0 <= a.sector.z1) return 1
            //if(player_height+jump > b.sector.z0 && b.sector.z0 <= a.sector.z1) return -1



            if(b.sector.z0 > a.sector.z0 && b.sector.z1 > a.sector.z1 && player_height+jump > a.sector.z0){
                if(ray.i==debug_ray && !printed) console.log(a.sector.name, a.out, b.sector.name, b.out, '1b')
                return 1
            }
            else if(b.sector.z0 > a.sector.z0 && b.sector.z1 > a.sector.z1 && player_height+jump < a.sector.z0){
                if(ray.i==debug_ray && !printed) console.log(a.sector.name, a.out, b.sector.name, b.out, '2a')
                return -1
            }
            //
            else if(a.sector.z0 > b.sector.z0 && a.sector.z1 > b.sector.z1 && player_height+jump > b.sector.z0){
                if(ray.i==debug_ray && !printed) console.log(a.sector.name, a.out, b.sector.name, b.out, '3a')
                return -1
            }
            if(a.sector.z0 > b.sector.z0 && a.sector.z1 > b.sector.z1 && player_height+jump < b.sector.z0){
                if(ray.i==debug_ray && !printed) console.log(a.sector.name, a.out, b.sector.name, b.out, '4b')
                return 1
            }
            else{
                if(ray.i==debug_ray && !printed) console.log(a.sector.name, a.out, b.sector.name, b.out, 'else')
                return +((a.dist) - (b.dist)).toFixed(2)
            }
        })
    }
    ...
    for(let ray of rays){
        ...
        for(let coll of ray.coll){
            if(coll.sector.z0 < player_height+jump && coll.sector.z1 < player_height+jump) draw_plane(coll, true)
            if(coll.sector.z1 > player_height+jump && coll.sector.z0 > player_height+jump) draw_plane(coll, false)
            drawWall(coll)
        }
    ...
    for(let [i, coll] of rays[debug_ray].coll.entries()){
        ...
        rayCtx.fillText(i, coll.dist/4, rayCanvas.height/2-coll.sector.z0/2-10)
    }
...
function draw_plane(coll, isTop){
    ...
    top_px = ((z-player_height-jump)/(coll.prev_dist*ray_cos))*plane_dist
    bot_px = ((z-player_height-jump)/(coll.next_dist*ray_cos))*plane_dist
    ceil_height = z-player_height
    ...
    for(let y=y0; y<y1; y++){
        ...
        image_x = Math.floor(posx + ray_rot_cos*d)*scale
        image_y = Math.floor(posy + ray_rot_sin*d)*scale
...
function drawSky(){
    for(y=Math.floor(render.height/2+head) ; y>0 ; y--){
        a = Math.floor(y*render.width)
        for(x=0 ; x<render.width ; x++){
            if(data[a+x]) continue
            data[a + x] = (255 << 24) | 255 << 16 | (200 << 8) | 100 //a,b,g,r
        }
    }
    for(y=Math.floor(render.height/2)+head ; y<render.height ; y++){
        a = Math.floor(y*render.width)
        y1 = y-Math.floor(render_height_2+head)
        for(x=0 ; x<render.width ; x++){
            if(data[a+x]) continue
...
function frame(time) {
    ...
    if(collisions && p_colls[0].dist<10){
        if(p_colls[0].sector.static){
            new_posx = Math.floor(posx)
            new_posy = Math.floor(posy)
        }else{
            if(player_height+jump<p_colls[0].sector.z0 && player_height+jump>p_colls[0].sector.z1){
                new_posx = Math.floor(posx)
                new_posy = Math.floor(posy)
            }else{
...
function customSort(arr, compareFunction) {
    ...
    for (let i = 0; i < clonedArray.length - 1; i++) {
      for (let j = 0; j < clonedArray.length - i - 1; j++) {
        ...
        const shouldSwap = compareFunction(clonedArray[j], clonedArray[j + 1]) > 0
        if (shouldSwap) {
          // Swap elements
