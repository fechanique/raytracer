    ...
    for(let ray of rays){ //hacer operaciones aquí no quita muchos frames
        ...
        for(let[index, sector] of sectors.entries()){
            if(inside_sector[sector.id]){
                ray.coll.push({id:sector.name, dist: 0, int: {}, texture: null, sector, face:null, sprite:null}) //fix missing spline fov
            }
        ...
        while(sorted_coll.length < ray.coll.length && iterations < 100){
            for(var coll of ray.coll){
                ...
                if(player.height+player.jump < coll.sector.z1){
                ...
                }else if(player.height+player.jump > coll.sector.z0){
                    if(ray.coll.find((e) => e.sector.id != coll.sector.id && !e.sorted 
...
function drawWall(coll){
    top_px = ((coll.sector.z0-player.height-player.jump)/(coll.dist*ray_cos))*cam.plane_dist
    bot_px = ((coll.sector.z1-player.height-player.jump)/(coll.dist*ray_cos))*cam.plane_dist
...
function draw_plane(coll, isTop){
    ...
    top_px = ((z-player.height-player.jump)/(coll.prev_dist*ray_cos))*cam.plane_dist
    bot_px = ((z-player.height-player.jump)/(coll.next_dist*ray_cos))*cam.plane_dist
    ceil_height = z-player.height
    if(top_px<bot_px) [top_px, bot_px] = [bot_px, top_px]

    y0 = Math.floor(Math.max(((game.height/2)-(top_px)+player.head), 0))
    y1 = Math.floor(Math.min(((game.height/2)-(bot_px)+player.head), game.height))
    cons_1 = ((ceil_height+player.height-player.height-player.jump)*cam.plane_dist)
    for(let y=y0; y<y1; y++){
