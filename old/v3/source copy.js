    ...
    for(let ray of rays){
        ...
        for(let i=0; i<ray.coll.length; i++){
            ...
            if(coll.sector.static){
                let size = ((coll.sector.ch-coll.sector.fh)/(coll.dist*Math.cos((ray.deg)*Math.PI/180)))*render.height
                let y0 = ((coll.sector.ch-jump)/(coll.dist*Math.cos((ray.deg)*Math.PI/180)))*render.height
